...
spi_image_collection <- ee_chirps_spi(chirps_tidy,window = 12,window_unit = "month",moi = c(1:12))
...
spi_bind <- spi_bind %>% mutate(
  ...
  parameter = case_when(
    parameter == "precipitation_sum_12_month_SPI" ~ "12-months SPI"))

write.csv(spi_bind,"gee_data/20kmsqdata/twelve_month_spi.csv")
